...
public class GoiMonDAO {
    ...
    public List<ThongKeDTO> LayDanhSachMonAnTheoNgayGoi(String ngaygoi) {
        String truyvan = " SELECT * FROM " + CreateDatabase.TB_CHITIETGOIMON + " ct, " + CreateDatabase.TB_MONAN + " ma, " + CreateDatabase.TB_GOIMON
                + " gm WHERE " + "ct." + CreateDatabase.TB_CHITIETGOIMON_MAGOIMON + " = gm."
                + CreateDatabase.TB_GOIMON_MAGOIMON + " AND ct."
                + CreateDatabase.TB_CHITIETGOIMON_MAMONAN + " = ma."
                + CreateDatabase.TB_MONAN_MAMON + " AND " + CreateDatabase.TB_GOIMON_NGAYGOI
                + " = '" + ngaygoi + "'";

        List<ThongKeDTO> thongKeDTOS = new ArrayList<ThongKeDTO>();
        Cursor cursor = database.rawQuery(truyvan, null);
        cursor.moveToFirst();
        while (!cursor.isAfterLast()) {
            ...
            thongKeDTO.setTenMonAn(cursor.getString(cursor.getColumnIndex((CreateDatabase.TB_MONAN_TENMONAN))));
            thongKeDTO.setGiaTien(cursor.getInt(cursor.getColumnIndex((CreateDatabase.TB_MONAN_GIATIEN))));
            thongKeDTO.setSoLuong(cursor.getInt(cursor.getColumnIndex((CreateDatabase.TB_CHITIETGOIMON_SOLUONG))));
            thongKeDTO.setHinhAnh(objectBitmap);

            thongKeDTOS.add(thongKeDTO);
            cursor.moveToNext();
        }
